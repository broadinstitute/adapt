...
class GuideSearcher:
    ...
    def __init__(self, aln, guide_length, mismatches, window_size, cover_frac,
        ...
        if window_size > aln.seq_length:
            raise ValueError("slice size must be less than the length of the alignment")
        ...
        self.primer_window_size = primer_window_size
    ...
    def _find_optimal_guide_in_window(self, start, slice_end, seqs_to_consider):
        ...
        search_end = slice_end

        #print("search_start: %i. search_end: %i" % (start, search_end))
    ...
    def _find_guides_that_cover_for_each_window(self, good_window_start, good_window_end):
        ...
        good_window_start = int(good_window_start)
        good_window_end = int(good_window_end)
    ...
    def find_guides_that_cover(self, out_fn, slice, sort=True, print_analysis=True):
        ...
        with open(out_fn, 'a+') as outf:
            ...
            for guides_in_window in guide_collections:
                ...
                guide_end = end - self.primer_length
                start = int(good_window_start)
                guide_seqs_str = ' '.join(sorted(list(guide_seqs)))
                line = [start, end, forward_primer, reverse_primer, count, score, guide_seqs_str]
...
class PrimerSearcher():
    ...
    def _construct_guide_memoized(self, start, seqs_to_consider):
        ...
        if (start in self._memoized_guides and
            ...
            try:
                p = self.aln.construct_guide(start, self.primer_length,
                        seqs_to_consider, self.primer_mismatches)
            except alignment.CannotConstructGuideError:
                p = None
            self._memoized_guides[start][seqs_to_consider_frozen] = p
            return p
    ...
    def _find_optimal_guide_in_window(self, start, seqs_to_consider):
        ...
        """
        assert start >= 0
        assert start + self.primer_window_size <= self.aln.seq_length

        # Calculate the end of the search (exclusive), which is the last
        # position in the window at which a guide can start; a guide needs to
        # fit completely within the window
        search_end = start + self.primer_window_size - self.primer_length + 1
    ...
    def _find_guides_that_cover_for_each_window(self):
        ...
        min_guides_in_cover_count = None
        guides = []
        for start in range(0, self.aln.seq_length - self.primer_window_size + 1):
    ...
    def find_primers_that_cover(self, out_fn, sort=True, print_analysis=True):
        ...
        # Returns 'usable_windows'. format = (window_start, window_end, forward_primer_seq, reverse_primer_seq)
        usable_windows = []

        for i in range(0, len(primer_positions)):
            for j in range(i+1,len(primer_positions)):
                # If the amplicon size (forward primer + sequence + reverse primer) meets length criteria, keep it
                if (primer_positions[j][1] - primer_positions[i][0]) <= self.window_size:
